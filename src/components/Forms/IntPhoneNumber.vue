		...
		<div class="int_phone_group">
			<div class="int_phone_select_warp">
				...
				<div class="int_phone_number_list" :style="`display: ${show_codes ? 'block' : 'none'}`">
					<input @keyup="searchCodes" value="" type="text" placeholder="Search"
						class=" form-control int_phone_number_search_input" />

					<div v-on:mouseleave="hideCodes" class="int_phone_number_country_list">
						<div v-for="(ele, idx) in country_codes" @click="() => updateExt(ele.code, ele.key, ele.flag)"
							:key="`phone-int-code-${idx}`" class="int_phone_countries_btn">
			...
			<input @keydown="updateParent" @change="updateParent" type="text" id="phone_number" name="phone_number"
				v-model="phone_num" v-mask="'################'" class="form-control int_phone_number"

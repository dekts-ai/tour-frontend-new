<template>
    <div v-if="display_form" class="accordion-item">
                        ...
                        <div class="row">
                            <div class="col-12 custom-form-wrap" :style="`height: ${display_height}px;`">
                                ...
                                <div v-for="(field, key) in fields">
                                    <div :id="field.id" :key="field.id" class="form-group">
                                        ...
                                        <div v-if="field.type === 'text'">
                                            ...
                                            <input :ref="field.id" :class="renderStyle(field.error)"
                                                :id="str.toId(field.id)" :type="field.type" v-model="field.value"
                                                class="form-control" />
                                        </div>
                                        ...
                                        <div v-if="field.type === 'textarea'">
                                            ...
                                            <textarea :ref="field.id" :class="renderStyle(field.error)"
                                                :id="str.toId(field.id)" :type="field.type" v-model="field.value"
                                        ...
                                        <div v-if="field.type === 'number'">
                                            ...
                                            <input :ref="field.id" :class="renderStyle(field.error)"
                                                :id="str.toId(field.id)" :type="field.type" :min="field.attrs.min"
                                                :max="field.attrs.max" v-model="field.value" class="form-control" />
                                        </div>

                                        <div v-if="field.type === 'checkbox'" class="custom-form-options-wrap">
                                            <input :ref="field.id" :class="renderStyle(field.error)"
                                                :id="str.toId(field.id)" :type="field.type" v-model="field.value"
                                                :value="field.default" class="form-check-input" />
                                        ...
                                        <div v-if="field.type === 'radio'" class="custom-form-options-wrap">
                                            <FormLabel :class="renderStyle(field.error)" :id="str.toId(field.id)"
                                                className="custom-form-label">
                                                <CustomFieldLabel :field="field" />
                                            </FormLabel>
                                            <div v-for="(option, idx) in field.attrs.options"
                                                :key="`radio-${key}-${idx}`">
                                                <input :ref="option.id" :value="option.value" :id="option.id"
                                                    :name="field.id" :type="field.type" v-model="field.value"
                                                    class="form-check-input" />
                                                <FormLabel :id="option.id" className="custom-radio-label">{{
                                                    str.toTitle(option.name) }}</FormLabel>
                                            </div>
                                        </div>

                                        <div v-if="field.type === 'select'">
                                            <FormLabel :class="field.error ? 'custom-from-error' : ''"
                                                :id="str.toId(field.id)" className="custom-form-label">
                                                <CustomFieldLabel :field="field" />
                                            </FormLabel>
                                            <select :ref="field.id" :id="str.toId(field.id)" v-model="field.value"
                                                class="form-select">
                                                <option v-for="option in field.attrs.options" :value="option.value">{{
                                                    option.name }}</option>
                                            </select>
                                        </div>
                                ...
                                <div v-if="display_submit" class="custom-from-actions">
                                    <input class="btn btn-success text-white float-end" type="button"
                                        @click="submitForm" value="Submit" />
                                </div>
                            </div>
                        </div>
                    </div>

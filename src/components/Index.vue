            ...
            <div class="row bradcumb-row" v-if="iframeStatus == false">
              <div class="col-12">
                <div class="bradcumb-main">
                  <ul>
                    <li class="home">
                      <a v-if="iframeStatus && data.package_id && form.affiliate_id"
                        :href="`${baseUrl}?pkg=${data.package_id}&aid=${form.affiliate_id}&iframe=${iframeStatus}`">
                        Home
                      </a>
                      <a v-else-if="iframeStatus && data.package_id"
                        :href="`${baseUrl}?pkg=${data.package_id}&iframe=${iframeStatus}`">
                        Home
                      </a>
                ...
                <div class="title-box">
                  ...
                  <div class="rating" v-if="form.package_id == 1">
                    <ul>
                      ...
                      <li class="info"><a href="javascript:void(0);" data-toggle="tooltip" data-placement="top"
                          title=""><img src="../assets/images/info.png"></a></li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            <div class="row payment-row">
              <div class="col-12">
                <div class="row booking-row">
                  ...
                  <div class="col-lg-6 col-md-12 text-center text-lg-end text-md-center text-sm-center">
                    <div class="info">
                      ...
                      <button class="tooltipbtn btn-danger" data-toggle="tooltip" data-placement="top" title="">Health &
                        Safety</button>
                    </div>
                  </div>
                </div>
                    ...
                    <div class="col-12">
                      <div class="row select-time">
                        <div class="col-12 col-md-4">
                          <datepicker v-model="form.date" :value="form.date" :inline="true"
                            :disabled-dates="disabledDates" :prevent-disable-date-selection="preventDisableDateSelection"
                        ...
                        <div class="col-12 col-md-8 mt-4 mt-md-0">
                          ...
                          <div class="radio-toolbar" v-if="dateTimeArr.length > 0">
                            <div class="time-item"
                              ...
                              v-for="name in dateTimeArr" :key="name.Id" @click="selectedSlot(name.Id, name.Time)">
                              ...
                              <text v-if="name.bookable_status == 'Open' && name.dd < name.seats" class="seats-free">{{
                                name.seats - name.dd }} AVAILABLE</text>
                      ...
                      <ul>
                        <li v-for="error in errors" :key="error" v-bind:class="{ 'text-danger': error }"
                              ...
                              <tbody v-for="tour in details.TourPkgRates" :key="tour.pkg_rate_id">
                                <tr>
                                  ...
                                  <td class="group" data-label="Select Group Of People">
                                    <select class="form-select people-group1" :name="'peoplegroup' + tour.pkg_rate_id"
                                      :id="'peoplegroup' + tour.pkg_rate_id">
                                      <option v-for="item in selectgrouppeoples" :value="item.value" :key="item.value">{{
                                        item.number }}</option>
                                    </select>
                                  </td>
                                  <td class="price" data-label="Price"
                                    v-if="peopleselects != '' && peopleselects != null">
                                    <span class="tag"><input type="hidden" name="amount" v-model="form.amount">{{
                                      tour.price }}</span>
                                  </td>
                ...
                <div class="row tour-start-row">
                  <div class="col-12 col-sm-12 col-md-6 duration" v-for="TourPkgDetails in details.TourPkgDetails"
                    :key="TourPkgDetails.pkg_rate_id">
                    <h2>Duration</h2>
